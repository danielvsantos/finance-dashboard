...
prisma.$use(async (params, next) => {
  ...
  if (model === 'Account' && (action === 'create' || action === 'update')) {
    ...
    if (!data.bank || data.bank.length < 2 || data.bank.length > 50) {
      throw new Error('Bank name must be between 2 and 50 characters.');
    }

    if (!data.currency || !/^[A-Z]{3}$/.test(data.currency)) {
      throw new Error('Invalid currency: Must be a valid ISO 3-letter currency code.');
    }

    if (!data.country || data.country.length < 2) {
      throw new Error('Country must be specified.');
    }

    if (!data.owner || data.owner.length < 2 || data.owner.length > 50) {
      throw new Error('Owner name must be between 2 and 50 characters.');
    }
  }
  ...
  if (model === 'StockPrice' && (action === 'create' || action === 'update')) {
    ...
    if (data.ticker?.length > 10) {
      throw new Error('Stock ticker must not exceed ten characters.');
    }
  }
